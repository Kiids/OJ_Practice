...
public:
    int findMinimum(int n, vector<int> left, vector<int> right) {
        ...
        return sum + min(l - lm + 1, r - rm + 1) + 1;
    }
};
...
public:    
	int findMinimum(int n, vector<int> left, vector<int> right){
		...
		//结果为有左右都有数量的手套序列的结果+有0存在的手套数+最后再加一肯定
		//就能保证了        
		return sum + min(left_sum-left_min+1, right_sum-right_min+1) + 1;
	} 
};

// 本题的意思是随意取出的手套至少可以形成一组组合的最少手套数量。题目给的两个数组对应位置表示同一 种颜色的左右手套数量。
// 对于非0递增序列a1,a2…an，要想最终取值覆盖每一个种类 n = sum(a1…an) - a1 + 1（也就是总数减去最小 值之后加一） 
// 所以对于左右手手套颜色都有数量的序列，想要覆盖每一种颜色，则最小数量leftsum = 左边 数量和 - 左边最小值 + 1， 
